...
#define STEXT_DIR "/home/koradiyd/stext"
#define TAR_FILE "/home/koradiyd/sclient/txt.tar" // Updated to use 'txt.tar'
...
void create_tar_file_and_send_to_smain(const char *tar_file);
...
void handle_client(int client_socket) {
    ...
    while (1) {
        // Receive command type length and command type
        ...
        // Receive path length and path
            ...
            if (strstr(filename, ".txt") != NULL) {
                ...
                if (replace_pos != NULL) {
                    ...
                    char new_path[512];
                    snprintf(new_path, sizeof(new_path), "%.*sstext/%s", (int)prefix_len, updated_path, replace_pos + 5);
                ...
                printf("Unsupported file type or command. Only .txt files are handled.\n");
            }
            ...
            char full_path[512];
            snprintf(full_path, sizeof(full_path), "%s/%s/%s", STEXT_DIR, path, filename);
        ...
        } else if (strcmp(command_type, "dtar") == 0) {
            printf("in tar function\n");
            create_tar_file_and_send_to_smain(TAR_FILE);
        } else {
...
// Function to create a tar file containing all .txt files in the stext directory
void create_tar_file_and_send_to_smain(const char *tar_file) {
    char command[1024];
    snprintf(command, sizeof(command), "find %s -type f -name '*.txt' | tar -cvf %s -T -", STEXT_DIR, tar_file);
    int status = system(command);
    if (status != 0) {
        fprintf(stderr, "Failed to create tar file: %s\n", tar_file);
    } else {
...
void upload_file(int client_socket, const char *file_path, const char *file_name) {
    char full_path[512];
    snprintf(full_path, sizeof(full_path), "/home/koradiyd/stext/%s/%s", file_path, file_name);

    // Create the directory if it doesn't exist
    char create_dir_cmd[BUFFER_SIZE];
    // Forming path to create directories from the command
    snprintf(create_dir_cmd, sizeof(create_dir_cmd), "mkdir -p '/home/koradiyd/stext/%s'", file_path);
